...
| `read_file` | 读取文件 | 读取 workspace 目录下的 `.txt` / `.md` / `.pdf` / `.docx` 文件 |
| `write_file` | 写入文件 | 准备写入文件，但必须用户确认后才真正写入 |
| `read_then_write` | 读取后生成文件 | 先读取源文件，再生成目标文件内容，并等待用户确认 |
| `summarize_file` | 文档摘要 | 先读取文件，再按摘要模板生成 Markdown 摘要，并等待用户确认 |
| `file_detail` | 文件详情 | 查看文件名、大小、修改时间和格式 |
...
| `unsupported_format` | 不支持格式 | 文件格式不是 `.txt` / `.md` / `.pdf` / `.docx` |
| `empty_file_name` | 文件名为空 | 用户没有提供有效文件名 |
| `multiple_matches` | 多候选匹配 | 文件名关键词匹配到多个候选文件，需要用户明确选择 |
...
预期 need_confirmation：false
预期结果：如果 workspace 中存在 test.xlsx，应提示不支持格式；如果不存在，应提示文件不存在
实际结果：待填写
是否通过：待测试
备注：
```
...
| Case 009 | 失败处理 | 读取 test.xlsx | read_file | unsupported_format / file_not_found | false | 待填写 | 待测试 |  |
...
## 十一、Tool Agent 3.0 文档工作流评测关注点

Tool Agent 3.0 在 2.0 多 Agent 工作流基础上，新增以下评测关注点：

```text
1. 文件上传是否成功保存到 workspace
2. PDF / DOCX 是否能正确读取
3. summarize_file 是否能生成摘要
4. summary_template 是否正确识别
5. history.jsonl 是否记录任务
6. run_eval.py 是否能生成 eval_results.md
```

建议手动评测时同时观察页面中的文件上传区域、摘要模板选择、历史任务列表和 `summary_template` / `generated_file_name` / `history_id` 状态字段。

---

## 十二、Tool Agent 3.2 文件名匹配与预览下载评测关注点

Tool Agent 3.2 在 3.0 文档工作流基础上，新增以下评测关注点：
...
4. 多个候选文件时是否提示候选列表
5. 确认写入后是否直接显示摘要预览
6. 是否可以下载生成的 summary.md
```

建议手动评测时同时观察页面中的 `tool_file_name`、`review_result`、`pending_file_name` 和“最近生成内容预览”区域。
...
如果 15 个测试用例中 12 个以上通过，并且 3.2 新增的文件名智能匹配、写入后摘要预览和下载能力表现稳定，则认为当前手动评测集可继续用于 Tool Agent 3.2。
---
