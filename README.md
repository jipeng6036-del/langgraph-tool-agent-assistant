...
系统基于 LangGraph 构建最小多 Agent 工作流，通过 Streamlit 提供可视化页面，支持用户输入自然语言任务，由 Planner Agent 规划任务，Tool Executor Agent 执行工具，Reviewer Agent 审查结果，Final Answer Agent 生成最终回复。
...
Planner Agent 规划任务
↓
Tool Executor Agent 执行工具
↓
Reviewer Agent 审查结果
↓
Final Answer Agent 生成最终回复
↓
保存 Agent 状态
↓
页面展示结果
...
Tool Agent 1.4 新增的手动评测集在 Tool Agent 2.0 中继续可用：
...
自动运行测试输入
自动记录实际 tool_action、error_type、need_confirmation、review_passed
生成评测结果报告
```
---
...
```text
基于 LangGraph 构建多 Agent 工具调用助手，将任务规划、工具执行、结果审查和最终回复拆分为 Planner Agent、Tool Executor Agent、Reviewer Agent 和 Final Answer Agent。系统支持根据用户自然语言任务自动选择 list_files、read_file、write_file、read_then_write 等工具，并实现写入前用户确认、状态保存与恢复、工具失败处理、手动评测集和审查结果记录。
```
